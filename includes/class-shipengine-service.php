...
class ShipEngine_Service {
	...
	public function build_package_plan(\WC_Order $order, array $package, int $package_number): array {
		...
		foreach ($candidates as $candidate) {
			$response = $this->request_rate($order, $candidate, $package);
	...
	protected function package_fits_box(array $package, array $box): bool {
		...
		return
			(float) $dimensions['length'] <= (float) $box['inner_length'] &&
	...
	protected function request_rate(\WC_Order $order, array $candidate, array $package): array {
		...
		if (is_wp_error($response)) {
			$this->log('ShipEngine request failed.', array(
		...
		if ($code < 200 || $code >= 300) {
			$this->log('ShipEngine returned a non-success response.', array(
				'order_id' => $order->get_id(),
				'status'   => $code,
				'body'     => $body,
			));
		...
		if (empty($rates)) {
			$this->log('ShipEngine returned no rates.', array(
				'order_id' => $order->get_id(),
				'body'     => $body,
			));
	...
	protected function get_ship_to_address(\WC_Order $order): array {
		return array(
			...
			'country_code'  => $order->get_shipping_country() ?: 'US',
